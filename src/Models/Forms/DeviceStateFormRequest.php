...
class DeviceStateFormRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        if (
            ...
            $rules = array_merge($rules, ['id' => ['required','integer','min:1','exists:'.config('wk-core.table.device-sensor.devices_states').',id']]);
        } elseif ($request->isMethod('post')) {
            $rules = array_merge($rules, ['id' => ['nullable','integer','min:1','exists:'.config('wk-core.table.device-sensor.devices_states').',id']]);
        }
    ...
    public function withValidator($validator)
    {
        $validator->after( function ($validator) {
            ...
            if (isset($data['identifier'])) {
                $result = config('wk-core.class.device-sensor.deviceState')::where('identifier', $data['identifier'])
                                ...
                                ->exists();
                if ($result)
                    $validator->errors()->add('identifier', trans('php-core::validation.unique', ['attribute' => trans('php-device-sensor::deviceState.identifier')]));
            }
        });
    }
}
