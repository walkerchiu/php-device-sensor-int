...
namespace WalkerChiu\DeviceSensor\Models\Observers;

class DeviceRegisterLangObserver
{
    /**
    ...
    {
        $query =
            config('wk-core.class.device-sensor.deviceRegisterLang')
                ::withTrashed()
        ...
        if (
            config('wk-device-sensor.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-device-sensor.lang_log')
            )
        ) {
